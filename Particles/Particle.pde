...
class Particle implements OneStateFunction {
  ...
  Particle() {
    ...
    direct=new Vector(NextDouble()-NextDouble(),NextDouble()-NextDouble(),NextDouble()-NextDouble());
    direct.Normalize();
    velocity=new Vector();
    ...
    result=new ParticleResult(1+(1+NextInt(4))*NextDouble(),1+(1+NextInt(5))*NextDouble());
  ...
  double Move(double t,double lim,double dampen,double spin){
    ...
    if(travelled>travels || r>lim || local.r<1.0){
      ...
      direct.Normalize();
      velocity=new Vector();
      local.x=0;local.y=0;local.z=0;local.r=startingRadius;
      in.ResetState();
      splash=1;
      travelled=0;
    }
